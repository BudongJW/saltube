...
> **《이런 말이 나오면 의심하세요》** — 특정 주장이 아니라 **논법 자체**를 먼저 가르치는 편.
> C 기둥(방법론)의 확장이며, §3의 "전이 가능성" 원리와 같은 이유로 효율이 높습니다.

**✅ 착수함** — [`content/fallacies.yaml`](../content/fallacies.yaml) 에 7개 가족을 정의했고,
주장 25건을 전부 덮습니다. PB01(반증 불가능) 대본 완성.

각 편이 파는 물건은 **한 문장짜리 질문**입니다. `claimctl validate` 가
접종 편에 그 문장이 실제로 실렸는지 검사하고, 없으면 발행을 막습니다 —
빠지면 그냥 또 하나의 주장 해체 편이 되기 때문입니다.

→ [01 전략](01-strategy.md) §6
...
|---|---|---|
| EP001 | 다의어 오류 | 47.5초 |
| EP002 | 조건 생략 인용 | 51.1초 |
| EP003 | 무한 후퇴 요구 | 53.2초 |
| EP004 | 선택적 회의 | 54.7초 |

재작성 과정에서 드러난 것 하나 — **오류명을 정확히 붙이려니 주장을 다시 읽게 됩니다.**
C004("가정에 의존하므로 믿을 수 없다")에 처음 붙인 `특수 사례 일반화`는
세인트헬렌스 하위 논증의 오류였지 주장 본체의 오류가 아니었습니다.
본체의 오류는 **선택적 회의** — 한 방법에만 증명 요구를 들이대면서
자기 논증(달 후퇴율 외삽)에는 훨씬 강한 가정을 쓰는 것입니다.

오류명 부여가 주장을 정밀하게 읽게 만드는 장치로도 작동합니다.
